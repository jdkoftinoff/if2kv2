...
/* @(#) $Id: minigzip.c 1679 2004-07-31 18:44:49Z jeffk $ */
...
int  gzip_main             OF((int argc, char *argv[]));
 ...
 */
static int gz_compress_mmap(in, out)
    FILE   *in;
    gzFile out;
{
    int len;
 ...
 */
static void gz_uncompress(in, out)
    gzFile in;
    FILE   *out;
{
    local char buf[BUFLEN];
 ...
 * original.
 */
static void file_compress(file, mode)
    char  *file;
    char  *mode;
{
 ...
 */
static void file_uncompress(file)
    char  *file;
{
